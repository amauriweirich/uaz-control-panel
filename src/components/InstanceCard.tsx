...
export function InstanceCard({ instance, onRefresh }: InstanceCardProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      toast({ title: 'Conexão iniciada', description: 'Escaneie o QR Code no WhatsApp' });
      onRefresh();
    } catch (error) {
      toast({ title: 'Erro', description: 'Falha ao conectar instância', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toast({ title: 'Erro', description: 'Falha ao desconectar instância', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGetQR = async () => {
    setLoading(true);
    try {
      const result = await api.getQRCode(instance.id);
      setQrCode(result.qrCode || result.base64 || result.data);
      setQrModalOpen(true);
    } catch (error) {
